...
@Component({
  ...
  template: `
    <div 
      ...
      <div *ngIf="selectedFile" class="mt-3 p-2 bg-secondary rounded d-flex justify-content-between align-items-center mx-auto" style="max-width: 300px;">
        <span class="text-truncate fw-500">{{ selectedFile.name }}</span>
        <button type="button" class="btn btn-sm text-danger" (click)="clearFile($event)">&times;</button>
      </div>
    </div>
  `,
  styles: [`
    ...
    .text-primary { color: var(--qt-navy); }
    .d-none { display: none; }
    .bg-secondary { background-color: #fff; border: 1px solid var(--border-color); }
