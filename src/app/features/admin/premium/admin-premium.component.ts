...
@Component({
  ...
  template: `
    <div class="container-fluid py-2">
        ...
        <table class="table mb-0">
          ...
          <tbody>
            <tr *ngFor="let req of requests">
              ...
              <td><span class="small text-muted">{{ req.orderId }}</span></td>
              <td>
                <span class="badge" 
                  [ngClass]="{
              ...
              <td class="text-right d-flex gap-2 justify-content-end">
                <button *ngIf="req.status === 'PENDING'" class="btn btn-sm btn-outline-primary" (click)="updateStatus(req, 'IN_PROGRESS')">Mark In Progress</button>
                <button *ngIf="req.status === 'IN_PROGRESS'" class="btn btn-sm btn-success text-white" (click)="openDeliverModal(req)">Deliver CV</button>
              </td>
            </tr>
      ...
      <div class="card p-4 shadow-sm border-none bg-white" *ngIf="selectedRequest">
        <div class="d-flex justify-content-between align-items-center mb-3">
          <h5 class="m-0 text-navy">Deliver Final CV to User ({{ selectedRequest.userEmail }})</h5>
          <button class="btn btn-sm btn-light border" (click)="selectedRequest = null">Cancel</button>
        </div>
        <form [formGroup]="deliverForm" (ngSubmit)="submitDelivery()">
          ...
          <div class="form-group mb-4">
            ...
            <textarea class="form-control" formControlName="feedback" rows="3" placeholder="Notes for the user..."></textarea>
          </div>
          <button type="submit" class="btn btn-primary" [disabled]="deliverForm.invalid || !deliveryFile || isLoading">
