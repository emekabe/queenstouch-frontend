    ...
    <div class="container py-5 min-vh-100">
      ...
      <div class="card p-0 shadow-sm border-none overflow-hidden">
        <table class="table mb-0">
          ...
          <tbody>
            <tr *ngFor="let cl of coverLetters">
              <td class="fw-500">{{ cl.jobTitle }}</td>
              <td>{{ cl.companyName }}</td>
              <td>{{ cl.createdAt | date:'mediumDate' }}</td>
              <td class="text-right">
                <button class="btn btn-sm btn-outline-primary me-2" (click)="downloadCL(cl.id, 'PDF')">PDF</button>
                <button class="btn btn-sm btn-outline-primary me-2" (click)="downloadCL(cl.id, 'DOCX')">DOCX</button>
                <button class="btn btn-sm text-danger btn-outline-danger" (click)="deleteCL(cl.id)">Delete</button>
              </td>
            </tr>
            <tr *ngIf="coverLetters.length === 0">
              <td colspan="4" class="text-center py-4 text-muted">You haven't generated any cover letters yet.</td>
            </tr>
          </tbody>
        </table>
      </div>
